...
export function CodeBlockEnhancer({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    pres.forEach((pre) => {
      ...
      btn.className =
        "absolute top-1.5 right-1.5 z-10 h-7 w-7 inline-flex items-center justify-center whitespace-nowrap text-xs transition-colors border border-input bg-background hover:bg-accent hover:text-accent-foreground";
