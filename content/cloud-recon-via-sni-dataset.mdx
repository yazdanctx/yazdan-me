---
title: پیدا کردن Cloud Asset با استفاده از دیتاست SNI
date: 2026-07-27
...
مثلا اگه که کاربر قصد داشته باشه به `api.dell.com` وصل بشه، مقدار اکستنشن مربوط به `SNI` داخل `ClientHello` این شکلی میشه:
...
ServerHello
        │
        ├── Select Certificate(api.example.com)
        ├── Send Certificate
        ▼
TLS Handshake Continues
```

نکته‌ای که این موضوع رو برای `Recon` ارزشمند می‌کنه اینه که مقدار `SNI` در پیام `ClientHello` ارسال میشه و سرور نیز بر اساس همون مقدار `Certificate` متناظر رو ارائه می‌کنه. بنابراین اگر یک اسکنر میلیون‌ها `IP` عمومی رو بررسی کنه و برای هر کدوم یک `TLS Handshake` انجام بده، می‌تونه `Hostname` ها و `Certificate`های مشاهده‌شده روی اون `IP`ها رو جمع‌آوری کنه. دقیقاً همین دیتاستیه که پروژه `Kaeferjaeger` به‌صورت هفتگی تولید و منتشر می‌کنه و بعدا میشه روی اون برای `Asset Discovery` جستجو انجام داد.

>اینجا گفتم که سرور بر اساس مقدار `SNI`، سرتیفیکیت مناسبو برمیگردونه ولی اگه بخوام دقیق تر بگم، اما این رفتار همیشگی نیست و بعضی سرورها همیشه `Default Certificate` را برمیگردونن.
...
## بررسی دیتاست Kaeferyeager

این `Kaeferyeager` یه گروه هکرن. بعد اینا میان هر هفته، میلیون ها `IP` متعلق به `Cloud Provider` هارو اسکن میکنن و با هر `IP` یه `TLS Handshake` میزنن و اطلاعات `SNI` و `Certificate` رو در میارن و در نهایت همه رو میذارن روی وبسایتشون و شما میتونی بری دانلود کنی.
...
خب تا اینجا ما فهمیدیم که چطور از طریق این دیتاستی که `kaeferyeager` بهمون میده، بریم `Cloud Assets` پیدا کنیم. 
...
اول میری رو یه کلون از این [ریپازیتوری](https://github.com/yazdanctx/kaeferyeager-sni-ip-sync/blob/main/sync.go) میگیری. داخل این ریپازیتوری یه اسکریپت گولنگ هست که میره `ipv4_merged_sni` هارو دانلود میکنه و همشونو میریزه داخل یدونه فایل تکست:
...
```
📁 /root/kaeferyeager-sni-ip-sync/output/dell_com/results_1785153512902783379.txt
...
با خوندن این رایت آپ یاد گرفتی که چطور با استفاده از دیتاست `kaeferyeager` ، بری و از `Cloud Provider` ها `asset` های جدید پیدا کنی.

نکته آخرم اینکه من روی سرور خودم یه `Cron job` ست کردم که هفته ای یکبار بره `kaeferyeager` رو چک بکنه و دیتا رو سینک کنه، شما هم میتونی اینکارو بکنی تا همیشه دیتاستت `up to date` باشه.